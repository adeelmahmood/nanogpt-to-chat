...
# data loader
train_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size, split="train", data_root="tinystories", master_process=master_process)
val_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size, split="val", data_root="tinystories", master_process=master_process)
...
for i in range(max_steps):
  ...
  # save checkpoint
  if i > 0 and (i % 250 == 0 or last_step) and master_process:
    save_checkpoint(
        f"./ckps/tst_model_{i:05d}.pt",
